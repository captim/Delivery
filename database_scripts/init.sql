...
ALTER TABLE `delivery_db`.`users` ADD INDEX `role_id_idx` (`role_id` ASC) VISIBLE;
ALTER TABLE `delivery_db`.`users`
    ADD CONSTRAINT `role_id` FOREIGN KEY (`role_id`)
    REFERENCES `delivery_db`.`roles` (`role_id`)
    ON DELETE RESTRICT ON UPDATE RESTRICT;

ALTER TABLE `delivery_db`.`users` ADD INDEX `shipping_address_id_idx` (`shipping_address_id` ASC) VISIBLE;
ALTER TABLE `delivery_db`.`users`
...
ALTER TABLE `delivery_db`.`requests` ADD INDEX `user_id_idx` (`user_id` ASC) VISIBLE;
ALTER TABLE `delivery_db`.`requests`
    ADD CONSTRAINT `user_id` FOREIGN KEY (`user_id`)
    REFERENCES `delivery_db`.`users` (`user_id`)
    ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE `delivery_db`.`requests` ADD INDEX `status_id_idx` (`status_id` ASC) VISIBLE;
ALTER TABLE `delivery_db`.`requests`
